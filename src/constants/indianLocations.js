...
export const INDIAN_DISTRICTS_MAP = {
  "Andaman and Nicobar Islands": [
  ...
  "Madhya Pradesh": [
    ...
    "Mandla", "Mandsaur", "Mauganj", "Morena", "Nagda", "Narmadapuram", "Narsinghpur", 
  ...
  "Punjab": [
    ...
    "Malerkotla", "Mansa", "Moga", "Muktsar (Sri Muktsar Sahib)", "Pathankot", 
    "Patiala", "Rupnagar (Ropar)", "Sahibzada Ajit Singh Nagar (Mohali)", "Sangrur", 
    "Shahid Bhagat Singh Nagar (Nawanshahr)", "Tarn Taran"
  ],
  "Rajasthan": [
    "Ajmer", "Alwar", "Anoopgarh", "Balotra", "Banswara", "Baran", "Barmer", 
    "Beawar", "Bharatpur", "Bhilwara", "Bikaner", "Bundi", "Chittorgarh", "Churu", 
    "Dausa", "Deeg", "Didwana-Kuchaman", "Dholpur", "Dudu", "Dungarpur", "Gangapur City", 
...
export const ALL_INDIAN_DISTRICTS = Array.from(
  new Set(Object.values(INDIAN_DISTRICTS_MAP).flat())
).sort((a, b) => a.localeCompare(b));

export function getDistrictsForState(stateName) {
  if (!stateName) return ALL_INDIAN_DISTRICTS;
  const direct = INDIAN_DISTRICTS_MAP[stateName];
  if (direct) return direct;
  
  // Case-insensitive match
  const lower = stateName.trim().toLowerCase();
  for (const [key, dists] of Object.entries(INDIAN_DISTRICTS_MAP)) {
