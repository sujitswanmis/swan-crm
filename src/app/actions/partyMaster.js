...
export async function createPartyMaster(partyData, tenantId = DEFAULT_TENANT_ID) {
  ...
  const payload = {
    ...
    tenant_id: tenantId,
    party_status: cleanPartyData.party_status || 'Draft',
    final_status: cleanPartyData.final_status || 'Draft',
    workflow_status: cleanPartyData.workflow_status || 'S00_Party_Master',
    next_step: cleanPartyData.next_step || (partyType === 'Distributor' ? 'S01_Distributor_Registration' : (partyType === 'Dealer' ? 'S02_Dealer_Registration' : 'S03_Sub_Dealer_Registration')),
